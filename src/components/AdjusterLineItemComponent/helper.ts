...
export const calculateRCV = (
  ...
  const replacementItem = state.lineItemDetail?.replacementItem;
  ...
  if (ItemDetails && ItemDetails.isReplaced) {
    ...
    const acceptedStandardCost = false;
    ...
    if (replacementItem) {
      ...
      if (subCategory) {
        ItemDetails = CalculateRCVWithSplCase(subCategory, Price, ItemDetails);
      }
